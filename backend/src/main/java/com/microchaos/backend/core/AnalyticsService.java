...
public class AnalyticsService {
    ...
    public Map<String, Object> dashboardOverview() {
        int totalServices = serviceRegistryService.list().size();
        List<com.microchaos.backend.model.Experiment> experiments = experimentService.listExperiments();
        List<ExperimentRun> runs = experimentService.listRuns();
        int totalExperiments = experiments.size();
        long activeRuns = runs.stream().filter(run -> run.getStatus() == com.microchaos.backend.model.RunStatus.RUNNING).count();
        long failedRuns = runs.stream().filter(run -> run.getStatus() == com.microchaos.backend.model.RunStatus.FAILED).count();
        double avgResilience = runs.stream().mapToDouble(ExperimentRun::getResilienceScore).average().orElse(0.0);
        ...
        response.put("recentRuns", runs.stream().sorted((a, b) -> Long.compare(b.getId(), a.getId())).limit(5).map(ExperimentRun::toMap).toList());
